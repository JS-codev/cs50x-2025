...
def index():
    ...
    # Iterate over stocks, add price and total values
    for stock in stocks:
        quote = lookup(stock["symbol"])
        stock["name"] = quote["name"]
        stock["price"] = float(quote["price"])
        stock["total_shares"] = int(stock["total_shares"])
...
def buy():
    ...
    if request.method == "POST":
        ...
        if not symbol:
            return apology("most provide symbol")
        elif not shares or not shares.isdigit() or int(shares) <= 0:
            return apology("most provide a positive integer number of shares")
...
def history():
    ...
    # look through database for user's transactions, ordered by most recent first
...
def register():
    ...
    if request.method == "POST":
        ...
        if not request.form.get("username"):
            ...
            return apology("confirm password error", 400)
        elif request.form.get("password") != request.form.get("confirmation"):
            return apology("passwords do not match", 400)

        # check for previous username
        ...
        # check for new inserted username
        rows = db.execute("SELECT * FROM users WHERE username = ?", request.form.get("username"))

        #remeber which user has log
        session["user_id"] = rows[0]["id"]
        ...
        for stock in stocks:
            if stock["symbol"] == symbol:
                if stock["total_shares"] < shares:
                    ...
                    #add sale to history table (delete shares according to user)
                    db.execute("INSERT INTO transactions (user_id, symbol, shares, price) VALUES (:user_id, :symbol, :shares, :price)", user_id=session["user_id"], symbol=symbol, shares=-shares,price=price)
